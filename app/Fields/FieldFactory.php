 ...
 *
 * Extensible via the `smart_product_options_addons_register_field_types` filter.
 *
 * @since      1.0.0
 * @package    SmartProductOptionsAddons
 * @subpackage SmartProductOptionsAddons/Fields
...
class FieldFactory {
	...
	private static function get_types() {
		if ( null === self::$types ) {
			 ...
			 */
			self::$types = apply_filters( 'smart_product_options_addons_register_field_types', self::$types );
		}
		return self::$types;
	}
	...
	public static function create( int $group_id, array $schema ) {
		...
		if ( ! isset( $types[ $type ] ) ) {
			smart_product_options_addons_log( "FieldFactory: Unregistered field type requested: '{$type}' in group {$group_id}", 'WARNING' );
			return null;
		}

		$class = $types[ $type ];
		if ( ! class_exists( $class ) ) {
			smart_product_options_addons_log( "FieldFactory: Class '{$class}' for type '{$type}' does not exist.", 'ERROR' );
			return null;
		}
